/* Задание CalcTree10: Построение, вычисление и преобразование дерева арифметического выражения */
...
#include <string>
#include <iostream>
#include <vector> // Для стеков при парсинге
 ...
 */
struct Node {
	NodeType type;     // Тип узла (оператор, число, переменная)
	char op = '\0';    // Символ оператора (если type == OPERATOR)
	int number = 0;  // Значение числа (если type == NUMBER)
	Node* left = nullptr;  // Указатель на левого потомка
	Node* right = nullptr; // Указатель на правого потомка
	...
	// Деструктор не нужен здесь, удаление будет управляться извне
};
...
#endif // EXPRESSION_TREE_EXPRESSION_TREE_H_